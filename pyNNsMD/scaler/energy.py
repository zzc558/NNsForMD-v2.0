...
class MaskedEnergyGradientStandardScaler(ScalerBase):
    def __init__(self,
                 scaler_module="energy",
                 use_energy_mean=True,
                 use_energy_std=True,
        ...
        super(MaskedEnergyGradientStandardScaler, self).__init__()
        self.use_energy_std = use_energy_std
        self.use_energy_mean = use_energy_mean
        self.use_x_std = use_x_std
        self.use_x_mean = use_x_mean
    ...
    def _generate_mask(self, energy):
        mask = K.cast(K.not_equal(energy, 0.0), dtype='float32').numpy()
        return mask
    ...
    def transform(self, x=None, y=None):
        ...
        if y is not None:
            if isinstance(y, list):
                ...
                raise ValueError("Transform for expected [energy, force] but got %s" % y)
            energy_mask = self._generate_mask(energy)
    ...
    def fit(self, x=None, y=None):
        ...
        if isinstance(y, list):
            ...
            raise ValueError("Transform for expected [energy, force] but got %s" % y)
        self.mask = self._generate_mask(y0)
        if self.use_x_mean:
            self.x_mean = np.mean(x)
        if self.use_x_std:
            self.x_std = np.std(x) + npeps
        if self.use_energy_mean:
            self.energy_mean = np.sum(y0 * self.mask, axis=0, keepdims=True) / np.sum(self.mask, axis=0, keepdims=True)
            #self.energy_mean = np.mean(y0, axis=0, keepdims=True)
        if self.use_energy_std:
            self.energy_std = np.sqrt(np.sum(np.square((y0 - self.energy_mean) * self.mask), axis=0, keepdims=True)/np.sum(self.mask, axis=0, keepdims=True)) + npeps
            #self.energy_std = np.std(y0, axis=0, keepdims=True) + npeps
    ...
    def load_weights(self, file_path):
        ...
        self.gradient_mean = np.array(indict['gradient_mean'])
        self.gradient_std = np.array(indict['gradient_std'])
        self.mask = tf.constant(indict['mask'], dtype='float64')
